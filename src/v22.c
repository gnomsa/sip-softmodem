...
void v22_init(struct v22 *v){memset(v,0,sizeof *v);v->tx_phase=0.0;v->answer_role=1;}
void v22_set_answer_role(struct v22*v,int answer){v->answer_role=answer!=0;}
...
void v22_generate(struct v22*v,int16_t*out,size_t n){for(size_t i=0;i<n;i++){if(v->tx_clock<=0){int a,b;if(v->tx_samples<7200){a=b=1;}else if(v->tx_samples<15200){a=scramble(v,1);b=scramble(v,1);}else{a=scramble(v,raw_bit(v));b=scramble(v,raw_bit(v));}v->tx_phase+=phase_change(a,b);v->tx_clock+=8000.0/600.0;}double hz=v->answer_role?2400.0:1200.0;double carrier=v->tx_phase+2*M_PI*hz*(double)v->tx_samples/8000.0;out[i]=(int16_t)(sin(carrier)*10000.0);v->tx_samples++;v->tx_clock-=1.0;}}
...
void v22_receive(struct v22*v,const int16_t*in,size_t n){double hz=v->answer_role?1200.0:2400.0;for(size_t k=0;k<n;k++){double x=in[k]/32768.0;v->rx_i+=x*cos(v->rx_phase);v->rx_q-=x*sin(v->rx_phase);v->rx_phase+=2*M_PI*hz/8000.0;if(v->rx_phase>=2*M_PI)v->rx_phase-=2*M_PI;v->rx_clock+=1;v->rx_samples++;if(v->rx_clock>=8000.0/600.0){symbol(v);v->rx_clock-=8000.0/600.0;}}}
