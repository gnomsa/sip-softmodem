...
static v34_modem_session_config v34_live_config(int answer_side,int maximum_rate)
{
    ...
    config.role=answer_side?V34_INFO_ANSWER_MODEM:V34_INFO_CALL_MODEM;
    config.info0.symbol_2743=true;config.info0.symbol_2800=true;config.info0.symbol_3429=true;
    config.info0.carrier_3000_low=true;config.info0.carrier_3000_high=true;
    config.info0.carrier_3200_low=true;config.info0.carrier_3200_high=true;
    config.info0.allow_3429=true;config.info0.maximum_symbol_rate_difference=5;
    config.info0.cme=true;config.info0.constellation_1664=true;
    config.info0.clock_source=answer_side?V34_CLOCK_RECEIVE:V34_CLOCK_INTERNAL;
    config.info0.acknowledge=answer_side!=0;
    config.allowed_symbols=V34_SYMBOL_ALL_MASK;
    config.allowed_rates=v34_rates_up_to((unsigned)maximum_rate);
    config.maximum_rate=(unsigned)maximum_rate;
    config.maximum_symbol_difference=5;
    config.trellis=V34_TRELLIS_64;config.expanded_shaping=true;
    config.sample_rate=8000;config.training_amplitude=9000.0;
    config.coordinate_scale=180.0;
    return config;
}
    ...
    while(!stopping) {
        ...
        if(fds[0].revents&POLLIN) {
            ...
            if(!strncmp(input,"SIP/2.0 ",8)) {
                ...
                invite_retransmit=sip_invite_should_retransmit(sr.status);
                if(sr.status>=100&&sr.status<200)continue;
        ...
        while(call && now>=next_tx) {
            int16_t pcm[160]; uint8_t alaw[160],packet[172];int alaw_ready=0;
            ...
            if(jitter_result>0){
                ...
                if(c.v8&&!v8_done){
                    if(!answer_side&&v8s.state==V8_WAIT){
                        ...
                        if(ans_observed&&was&&!is&&!ansam_present(&ansam_rx)){v8_done=1;modem_started=1;ans_complete=1;if(c.speed==2400&&!standard_v34)v22bis_start_handshake(&modem22bis,0);}
                    }
                    if(!v8_done){
            ...
            if(c.v8&&!v8_done&&(int)v8s.state!=v8_last_state){
                ...
                else if(v8s.state==V8_CJ)v8_cj_pending=1;
                if(bits)v8_fsk_set_sequence(&v8tx,v8_menu_bits,bits);
            }
            if(c.v8&&!v8_done&&!answer_side&&v8s.state==V8_CJ&&v8_cj_pending&&v8tx.tx_at%10==0){size_t bits=v8_encode_cj(v8_menu_bits,sizeof v8_menu_bits);v8_fsk_set_sequence(&v8tx,v8_menu_bits,bits);v8_cj_pending=0;v8_cj_active=1;v8_state_started=now_ms();}
            if(c.v8&&!v8_done&&!answer_side&&v8s.state==V8_CJ&&v8_cj_active&&now_ms()-v8_state_started>=100){v8_cj_active=0;v8_session_cj(&v8s);}
            if(c.v8&&!v8_done&&(v8s.state==V8_SELECTED||v8s.state==V8_FAILED)){
                v8_done=1;modem_started=1;ans_complete=1;
                if(standard_v32)v42_v32_set_lapm(&modem32std,
                    v8s.state==V8_SELECTED&&v8s.joint.lapm);
                if(c.speed==2400&&!standard_v34){v22bis_start_handshake(&modem22bis,answer_side);if(answer_side)v22bis_answer_sequence_complete(&modem22bis);}
            ...
            if(!alaw_ready)pcma_encode_buffer(pcm,alaw,160);
            size_t length=rtp_build(&tx,alaw,160,packet,sizeof packet);
        ...
        if(call&&standard_v32&&modem_started){
            if(v32_log_retrain!=(int)modem32std.physical.retrain.state){
                v32_log_retrain=(int)modem32std.physical.retrain.state;
                fprintf(stderr,"V.32 retrain state %d AB/CD %u/%u E-words %u B1-wait %u\n",
                        v32_log_retrain,
                        modem32std.physical.retrain.ab_count,
                        modem32std.physical.retrain.cd_count,
                        modem32std.physical.e_rx.words,
                        modem32std.physical.bis_rx_reject_wait_symbols);
            }
            ...
            if(physical!=v32_log_connected||
               (int)modem32std.lapm.state!=v42_log_state){
                v32_log_connected=physical;
                v42_log_state=(int)modem32std.lapm.state;
                fprintf(stderr,"V.32 physical %s marking tx/rx %u/%u, V.42 state %d\n",
