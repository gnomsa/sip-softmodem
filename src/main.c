...
int main(void) {
    ...
    while(!stopping) {
        ...
        while(call && now>=next_tx) {
            ...
            if(jitter_result>0){
                ...
                if(c.v8&&!v8_done){
                    ...
                    if(standard_v32){v42_v32_receive(&modem32std,rx,160);got=v42_v32_read(&modem32std,bytes,sizeof bytes);}
        ...
        int negotiated=c.speed==2400?v22bis_selected_rate(&modem22bis):standard_v32?v42_v32_rate(&modem32std):c.speed;
        int trained=modem_started&&(c.speed==2400?v22bis_connected(&modem22bis):standard_v32?v42_v32_connected(&modem32std):((answer_side||ans_complete)&&now-call_started>5200));
