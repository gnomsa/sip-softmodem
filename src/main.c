...
int main(void) {
    ...
    c.speed=select_speed(c.protocols,c.max_rate);if(!c.speed){fprintf(stderr,"no implemented protocol enabled (this build: V21,V22,V22bis)\n");return 2;}
    ...
    struct v21 modem; struct v22 modem22; struct v22bis modem22bis; v21_init(&modem);v22_init(&modem22);v22bis_init(&modem22bis); struct rtp_sender tx={(uint16_t)rand(),(uint32_t)rand(),(uint32_t)rand()};
    ...
    while(!stopping) {
        ...
        while(call && now>=next_tx) {
            ...
            else if(media_samples<24600) memset(pcm,0,sizeof pcm); else if(c.speed==2400)v22bis_generate(&modem22bis,pcm,160);else if(c.speed==1200)v22_generate(&modem22,pcm,160);else v21_generate(&modem,pcm,160);
