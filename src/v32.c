...
void v32_generate(struct v32*v,int16_t*out,size_t n){for(size_t k=0;k<n;k++){if(v->tx_clock<=0){double i,q;levels(v,take(v),&i,&q);double norm=v->bits==2?9000.0:3200.0;v->tx_i=i*norm;v->tx_q=q*norm;v->tx_clock+=8000.0/2400.0;}double p=2*M_PI*1800.0*(double)v->tx_samples/8000.0;out[k]=(int16_t)(v->tx_i*cos(p)-v->tx_q*sin(p));v->tx_samples++;v->tx_clock-=1;}}
static int axis(double x){if(x<-6400)return 0;if(x<0)return 1;if(x>6400)return 2;return 3;}
void v32_receive(struct v32*v,const int16_t*in,size_t n){for(size_t k=0;k<n;k++){double p=2*M_PI*1800.0*(double)v->rx_samples/8000.0,x=in[k],c=cos(p),s=sin(p);v->rx_i+=x*c;v->rx_q-=x*s;v->rx_cc+=c*c;v->rx_ss+=s*s;v->rx_cs+=c*s;v->rx_samples++;v->rx_clock++;if(v->rx_clock>=8000.0/2400.0){double det=v->rx_cc*v->rx_ss-v->rx_cs*v->rx_cs;double i=v->rx_i,q=v->rx_q;if(fabs(det)>1e-9){i=(v->rx_ss*v->rx_i+v->rx_cs*v->rx_q)/det;q=(v->rx_cs*v->rx_i+v->rx_cc*v->rx_q)/det;}int code;if(v->bits==2)code=(i<0?1:0)|(q<0?2:0);else code=(axis(i)<<2)|axis(q);for(int j=v->bits-1;j>=0;j--)uart(v,(code>>j)&1);v->rx_i=v->rx_q=v->rx_cc=v->rx_ss=v->rx_cs=0;v->rx_clock-=8000.0/2400.0;}}}
